...
manifest="$repo_root/Server/Kernel/Manifests/linux-kernel.json"

usage() {
  cat <<'EOF'
Usage: kernel_fetch.sh [status|fetch|verify|extract|all] [target-id]

Targets are defined in Server/Kernel/Manifests/linux-kernel.json.
Default target: linux-longterm.

Generated files are stored under Server/Kernel/Generated/.
EOF
}
...
generated_dir="$repo_root/Server/Kernel/Generated"
...
extract() {
  ...
  mkdir -p "$repo_root/Server/Kernel/Generated/src"
  if [[ -d "$src_root" ]]; then
    echo "kernel-fetch: source already extracted: $src_root"
    return 0
  fi
  echo "kernel-fetch: extracting $archive_path"
  tar -xJf "$archive_path" -C "$repo_root/Server/Kernel/Generated/src"
}
