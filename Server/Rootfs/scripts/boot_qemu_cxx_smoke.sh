...
log="$log_dir/boot-cxx-smoke.log"
timeout_seconds="${RVS_BOOT_CXX_SMOKE_TIMEOUT:-45}"
...
if grep -q "MixtarRVS v0" "$log" &&
  ...
  echo "boot-cxx-smoke: ok"
  echo "boot-cxx-smoke: log=$log"
  exit 0
fi

echo "boot-cxx-smoke: failed rc=$rc" >&2
echo "boot-cxx-smoke: log=$log" >&2
tail -80 "$log" >&2 || true
exit 1
