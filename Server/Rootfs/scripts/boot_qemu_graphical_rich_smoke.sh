...
log="$log_dir/boot-graphical-rich-smoke.log"
timeout_seconds="${RVS_BOOT_GRAPHICAL_RICH_SMOKE_TIMEOUT:-75}"
...
if grep -q "MixtarRVS v0" "$log" &&
  ...
  echo "boot-graphical-rich-smoke: ok"
  echo "boot-graphical-rich-smoke: log=$log"
  exit 0
fi

echo "boot-graphical-rich-smoke: failed rc=$rc" >&2
echo "boot-graphical-rich-smoke: log=$log" >&2
tail -140 "$log" >&2 || true
exit 1
