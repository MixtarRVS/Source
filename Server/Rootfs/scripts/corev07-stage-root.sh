...
kernel_profile_source="$repo_root/Server/Kernel/Manifests/corev07-kernel-profile.json"
stage_root="$generated_dir/corev07-root"
efi_stage="$generated_dir/corev07-efi"
efi_source="$generated_dir/corev07-efi-build/MixtarRVS-$VERSION.efi"
...
usage() {
    cat <<EOF
usage: corev07-stage-root.sh [plan|stage|verify] [--root PATH] [--efi-stage PATH] [--efi-source PATH]

Defaults:
  root:      $stage_root
  efi-stage: $efi_stage

This script stages CoreV07 inside Server/Rootfs/Generated only.
...
require_safe_generated_path() {
    ...
    case "$resolved_parent/" in
        "$generated_dir"/*|"$generated_dir/")
            ;;
        *)
            fail "refusing path outside Server/Rootfs/Generated: $path"
            ;;
    esac
...
build_corev07_sshd_service() {
    ssh_dir="$stage_root/System/Networking/SSH"
    src="$generated_dir/corev07-mixtar-sshd-service.c"
    cc_bin="${CC:-}"
    if [ -z "$cc_bin" ]; then
...
static void attach_log(void) {
    ...
    fd = open("/Volumes/ESP/EFI/MixtarRVS/CoreV07-sshd.log", O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if (fd < 0) {
...
int main(void) {
    ...
    attach_log();
    fprintf(stderr, "mixtar-sshd-service: preparing CoreV07 recovery root\n");
    ...
    bind_required("/System/UI", "/System/Networking/SSH/Root/Native/System/UI");
...
C
    ...
    note "rebuilt CoreV07 /System/Networking/SSH/mixtar-sshd-service"
}

write_corev07_networking_service() {
    service="$stage_root/System/Networking/start-networking"
    src="$generated_dir/corev07-mixtar-networking-service.c"
    cc_bin="${CC:-}"
    if [ -z "$cc_bin" ]; then
        if command -v musl-gcc >/dev/null 2>&1; then
            cc_bin=musl-gcc
...
static void open_log(void) {
    ...
    log_fd = open("/Volumes/ESP/EFI/MixtarRVS/CoreV07-last.log", O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if (log_fd < 0) {
        mkdir_one("/System/Logs");
        log_fd = open("/System/Logs/CoreV07-last.log", O_WRONLY | O_CREAT | O_TRUNC, 0644);
    }
}
...
enum {
    COREV07_SSH_PROBE_ATTEMPTS = 45,
    COREV07_SSH_PROBE_INTERVAL_SECONDS = 2
};
...
static void run_ssh_selftest(void) {
    int fd = open_named_log("CoreV07-ssh-selftest.log");
    if (fd < 0) {
        log_line("networking: cannot open CoreV07-ssh-selftest.log: %s", strerror(errno));
        return;
    }
    dprintf(fd, "MixtarRVS CoreV07 SSH self-test\n");
    dprintf(fd, "port=22\n");
    int loopback_rc = 1;
    int native_rc = 1;
    int completed_attempt = 0;
    for (int attempt = 1; attempt <= COREV07_SSH_PROBE_ATTEMPTS; attempt++) {
        ...
        sleep_seconds(COREV07_SSH_PROBE_INTERVAL_SECONDS);
    }
...
static int run_icmp_probe_to_fd(int fd, const char *target) {
    ...
    pkt.seq = htons(1);
    memcpy(pkt.payload, "MixtarRVS-CoreV07-native-icmp", 29);
...
static void run_ping_test(void) {
    int fd = open_named_log("CoreV07-ping.log");
    if (fd < 0) {
        log_line("networking: cannot open CoreV07-ping.log: %s", strerror(errno));
        return;
    }
    dprintf(fd, "MixtarRVS CoreV07 network ping test\n");
...
int main(void) {
    open_log();
    log_line("MixtarRVS CoreV07 networking service started");
...
EOF
    ...
    write_corev07_networking_service
    build_corev07_sshd_service
    note "staged CoreV07 recovery networking and SSH"
}
...
stage_corev07_pid1() {
    ...
    sqlite_pid1_o="$sqlite_pid1_dir/sqlite3-corev07.o"
    generated_c="$generated_dir/corev07-mixtar-init.c"
...
write_contract_marker() {
    dst="$stage_root/System/Configuration/CoreV07.contract"
    cat > "$dst" <<EOF
version=$VERSION
...
executor_source_path=Server/Runtime/Executor/mixtar_executor.ail
...
system_ui_shell=/System/UI/Shell
system_ui_shell_apx=/System/UI/Shell/MixtarShell.apx
...
write_apx_seed() {
    apx_root="$stage_root/System/UI/Shell/MixtarShell.apx"
    mkdir -p "$apx_root/Program" "$apx_root/Icon" "$apx_root/Resources" "$apx_root/Data"

    cat > "$apx_root/Program/MixtarShell" <<'EOF'
#!/System/Shells/zsh
print -r -- "MixtarShell APX"
EOF
    chmod 0755 "$apx_root/Program/MixtarShell"
...
apx_root = pathlib.Path(sys.argv[1])
db_path = apx_root / "MixtarShell.config"
if db_path.exists():
    db_path.unlink()
db = sqlite3.connect(db_path)
db.executescript(
...
db.executemany(
    ...
    [
        ("app.id", "org.mixtar.shell.MixtarShell"),
        ("app.name", "MixtarShell"),
        ("app.version", "0.1"),
        ("entry.type", "native"),
        ("entry.path", "Program/MixtarShell"),
        ("runtime", "mixtar"),
...
write_db(
    "Networking.config",
    [
        ("networking.mode", "recovery-corev07"),
...
write_security_config() {
    ...
    python3 - "$config_dir" "$auth_dir" <<'PY'
import pathlib
import sqlite3
import sys

config_dir = pathlib.Path(sys.argv[1])
auth_dir = pathlib.Path(sys.argv[2])
...
write_db(
    ...
    [
        ("security.path", "/System/Security"),
        ("security.auth", "/System/Security/Auth"),
        ("security.runtime", "/System/Runtime/Security"),
        ("security.auth.socket", "/System/Runtime/Security/Auth.sock"),
        ("admin.mode", "session-token"),
        ("admin.command", "admin"),
        ("admin.session.title", "Administrator: Mixtar Terminal"),
        ("admin.session.prompt", "unchanged"),
        ("admin.session.user", "vxz"),
        ("admin.token.ttl.seconds", "300"),
        ("admin.token.inherits_to_children", "false"),
        ("admin.actions.logged", "true"),
        ("sudo.default", "false"),
        ("ui.required", "false"),
    ],
    7,
...
write_db(
    "AdminSession.config",
    [
        ("session.kind", "administrator"),
        ("title", "Administrator: Mixtar Terminal"),
        ("visible.status", "title"),
        ("prompt", "unchanged"),
        ("user.display", "vxz"),
        ("token.source", "/System/Runtime/Security/Tokens"),
        ("auth.socket", "/System/Runtime/Security/Auth.sock"),
        ("timeout.seconds", "300"),
        ("exit.command", "exit-admin"),
    ],
    7,
)
(auth_dir / "Auth.contract").write_text(
    "\n".join(
        [
            "auth.mode=session-token",
            "auth.socket=/System/Runtime/Security/Auth.sock",
            "auth.policy=/System/Configuration/Security/Policy.config",
            "admin.command=admin",
            "admin.session.title=Administrator: Mixtar Terminal",
            "admin.session.prompt=unchanged",
            "sudo.default=false",
...
write_pid1_config() {
    ...
    python3 - "$stage_root" "$VERSION" "$KERNEL_VERSION" <<'PY'
import pathlib
import sqlite3
import sys
...
config_dir = root / "System/Configuration"
...
root_dirs = [
    ...
    "/System/Drivers/Linux/RT",
    f"/System/Drivers/Linux/RT/{kernel_version}",
    "/System/EFI",
    ...
    f"/System/Kernel/Linux/RT/{kernel_version}",
    "/System/Libraries",
    "/System/Logs",
    "/System/Networking",
    "/System/Networking/Core",
    "/System/Networking/Core/Runtime",
    "/System/Networking/WiFi",
    "/System/Networking/WiFi/Runtime",
    "/System/Networking/WiFi/bin",
    "/System/Networking/WiFi/Root",
    "/System/Networking/SSH",
    "/System/Networking/SSH/Runtime",
    "/System/Networking/SSH/bin",
    "/System/Networking/SSH/sbin",
    "/System/Networking/SSH/libexec",
    "/System/Networking/SSH/Root",
    "/System/Process",
    "/System/Resources",
    "/System/Runtime",
...
users = [
    ("vxz", 1000, 1000, "/Users/vxz", "/System/Shells/zsh"),
    ("Administrator", 1001, 1001, "/Users/Administrator", "/System/Shells/zsh"),
]
chmods = [
    ("public-tmp", "/Temporary"),
    ("private-dir", "/Users/vxz"),
    ("private-dir", "/Users/Administrator"),
]
mounts = [
    ...
    ("tmpfs", "/System/Runtime", "tmpfs", 0o755),
...
meta = [
    ...
    ("session.config", "/System/UI/Session.config"),
    ("ui.namespace", "/System/UI"),
    ("ui.graphical.enabled", "false"),
    ("runtime.executor", "/System/Runtime/Executor"),
    ("runtime.executor.source", "AILang"),
    ("runtime.executor.source.path", "Server/Runtime/Executor/mixtar_executor.ail"),
    ("apx.executor", "/System/Runtime/Executor"),
    ("applications.user", "/Applications"),
    ("applications.mode", "user-visible-only"),
    ("system.ui.shell", "/System/UI/Shell"),
    ("system.ui.shell.apx", "/System/UI/Shell/MixtarShell.apx"),
    ("stage.scope", "generated-only"),
    ("boot.deploy", "disabled"),
    ("efi.mutation", "disabled"),
    ("config.path", "/System/Configuration"),
    ("config.mode", "sqlite-primary"),
    ("userland.path", "/System/Userland"),
    ("userland.mode", "command-root"),
    ("tools.path", "/System/Tools"),
    ("tools.mode", "admin-only"),
    ("drivers.path", "/System/Drivers"),
    ("drivers.mode", "store-only"),
    ("networking.service", "/System/Networking/start-networking"),
    ("networking.ping.group_range.path", "/System/Process/sys/net/ipv4/ping_group_range"),
    ("networking.ping.gid.min", "0"),
    ("networking.ping.gid.max", "1000"),
    ("persistence.enabled", "true"),
    ("persistence.device", "/System/Devices/nvme0n1p3"),
    ("persistence.fstype", "ext4"),
    ("persistence.mount", "/Volumes/MixtarRoot"),
    ("persistence.users.source", "/Volumes/MixtarRoot/Users"),
    ("persistence.users.target", "/Users"),
    ("persistence.configuration.source", "/Volumes/MixtarRoot/System/Configuration"),
    ("persistence.configuration.target", "/System/Configuration"),
    ("persistence.configuration.database", "/Volumes/MixtarRoot/System/Configuration/MixtarRVS.config"),
    ("networking.config", "/System/Configuration/Networking/Networking.config"),
    ("wifi.config", "/System/Configuration/Networking/WiFi.config"),
    ("regulatory.db", "embedded"),
    ("security.path", "/System/Security"),
    ("security.policy", "/System/Configuration/Security/Policy.config"),
    ("security.auth", "/System/Security/Auth"),
    ...
    ("users.shell.default", "/System/Shells/zsh"),
...
for _, _, _, home, _ in users:
    (root / home.lstrip("/")).mkdir(parents=True, exist_ok=True)

db_path = config_dir / "MixtarRVS.config"
if db_path.exists():
    db_path.unlink()
db = sqlite3.connect(db_path)
db.executescript(
    ...
    CREATE TABLE user(name TEXT COLLATE BINARY PRIMARY KEY, uid INTEGER NOT NULL UNIQUE, gid INTEGER NOT NULL, home TEXT NOT NULL, shell TEXT NOT NULL);
...
db.executemany("INSERT INTO user(name, uid, gid, home, shell) VALUES (?, ?, ?, ?, ?)", users)
...
init_lines.extend(f"user={name}:{home}:{shell}" for name, _uid, _gid, home, shell in users)
...
(config_dir / "MixtarRVS.init").write_text("\n".join(init_lines) + "\n", encoding="utf-8")
(config_dir / "mixtar-release").write_text(
    "\n".join(
        [
            "NAME=MixtarRVS",
            f"VERSION={version}",
...
(config_dir / "hostname").write_text("MixtarRVS\n", encoding="utf-8")

zsh_config_dir = config_dir / "ZSH"
zsh_config_dir.mkdir(parents=True, exist_ok=True)
zsh_db_path = zsh_config_dir / "ZSH.config"
if zsh_db_path.exists():
    zsh_db_path.unlink()
zsh_db = sqlite3.connect(zsh_db_path)
zsh_db.executescript(
    """
    PRAGMA page_size=1024;
    PRAGMA journal_mode=OFF;
    PRAGMA synchronous=OFF;
...
zsh_db.executemany(
    "INSERT INTO setting(key, value) VALUES (?, ?)",
    [
        ("shell.path", "/System/Shells/zsh"),
        ("runtime.path", "/System/Shells/Runtime"),
        ("path", "/System/Shells:/System/Userland"),
        ("prompt", "${USER:-User}@${MIXTAR_SYSTEM_NAME:-MixtarRVS}:%~> "),
        ("history.file", "${HOME}/.zsh_history"),
    ],
)
zsh_db.execute("PRAGMA user_version=5")
zsh_db.commit()
zsh_db.execute("VACUUM")
zsh_db.close()

for user_name, user_uid, user_gid, home, _shell in users:
    home_dir = root / home.lstrip("/")
    home_dir.mkdir(parents=True, exist_ok=True)
    (home_dir / ".zshenv").write_text(
        "\n".join(
            [
    ...
    (home_dir / ".zshrc").write_text(
        "\n".join(
            [
                ...
                "for key_sequence in '^[[[A' '^[[[B' '^[[[C' '^[[[D' '^[[[E' '^[OP' '^[OQ' '^[OR' '^[OS' '^[[15~' '^[[17~' '^[[18~' '^[[19~' '^[[20~' '^[[21~' '^[[23~' '^[[24~'; do bindkey \"$key_sequence\" mixtar-ignore-function-key; done",
                "unset key_sequence",
                "PROMPT='%F{green}${USER:-" + user_name + "}@${MIXTAR_SYSTEM_NAME:-MixtarRVS}%f:%F{blue}%~%f> '",
            ]
        )
        + "\n",
        encoding="utf-8",
...
write_kernel_profile() {
    ...
    if [ -f "$kernel_profile_source" ]; then
        ...
        cat > "$dst" <<EOF
{
  "name": "MixtarRVS CoreV07 RT kernel profile",
  "version": "$VERSION",
  "kernel": {
    "family": "Linux",
    "profile": "RT",
...
stage_layout() {
    mkdir -p \
        ...
        "$stage_root/System/UI/Fonts" \
        "$stage_root/System/UI/Icons" \
        "$stage_root/System/UI/Sessions" \
        "$stage_root/System/UI/Shell" \
        "$stage_root/System/UI/Themes" \
...
root = pathlib.Path(sys.argv[1])
ui_dir = root / "System/UI"
ui_dir.mkdir(parents=True, exist_ok=True)
db_path = ui_dir / "Session.config"
if db_path.exists():
    db_path.unlink()
db = sqlite3.connect(db_path)
db.executescript(
...
db.executemany(
    ...
    [
        ...
        ("apx.executor", "/System/Runtime/Executor"),
        ("applications.user", "/Applications"),
        ("runtime.sessions", "/System/Runtime/Sessions"),
        ("runtime.display", "/System/Runtime/Display"),
    ],
)
db.execute("PRAGMA user_version=1")
db.commit()
db.execute("VACUUM")
db.close()
(ui_dir / "Session.contract").write_text(
    "\n".join(
        [
            "session.mode=console",
            "session.user=vxz",
...
PY
    ...
    if [ -n "$efi_source" ] && [ -f "$efi_source" ]; then
        ...
        note "copied EFI source -> CoreV07 $VERSION.efi"
        note "copied EFI provenance -> CoreV07 $VERSION.efi.provenance"
...
case "$cmd" in
  status)
    ...
    print -r -- "  Reason: CoreV08 defines status plumbing; upstream checking is later."
    ;;
  sources)
...
case "$cmd" in
  status)
    ...
    ;;
  about)
    print -r -- "MixtarRVS CoreV08 is the no-UI foundation for MixtarRVS 1.0."
...
PY
    ...
    note "installed CoreV08 system status foundation"
}
build_console_setup() {
...
stage() {
    ...
    write_global_zsh_startup
    augment_console_configuration
    note "staged CoreV07 root at $stage_root"
    note "staged CoreV07 EFI mirror at $efi_stage"
}
...
case "$cmd" in
    ...
    stage)
        ...
        ;;
    verify)
        sh "$script_dir/corev08-keyboard-verify.sh" --root "$stage_root"
        exec "$script_dir/corev07-verify.sh" --root "$stage_root" --efi-stage "$efi_stage"
        ;;
esac
