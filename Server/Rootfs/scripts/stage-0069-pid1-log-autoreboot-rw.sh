...
ROOTFS_GEN=/System/Generations/0022-rootfs-image-pid1-log-autoreboot-rw
...
INITRD_CAND=/System/Initramfs/Candidates/0069-pid1-log-autoreboot-rw-candidate-no-install
INITRD_OUT=$INITRD_CAND/initramfs.img

REPORT=/System/Base/Closure/0069-pid1-log-autoreboot-rw.report
ROOT_WORK=/tmp/mixtar-rootfs-0022.$$
INIT_WORK=/tmp/mixtar-initramfs-0069.$$
VERIFY=/tmp/mixtar-0069-verify.$$

ESP_DEV=/dev/nvme0n1p1
ESP_MOUNT=/tmp/mixtar-esp-0069
ESP_INITRD_NAME=initrd-mixtar-candidate-0069.img

LABEL="MixtarRVS RT Candidate 0069 pid1 log autoreboot rw"
LOADER="\\EFI\\mixtarrvs-rt\\vmlinuz.efi"
INITRD="\\EFI\\mixtarrvs-rt\\initrd-mixtar-candidate-0069.img"
ROOTFS_ARG=/System/Generations/0022-rootfs-image-pid1-log-autoreboot-rw/rootfs.squashfs
...
sudo -n unsquashfs -d "$ROOT_WORK/root" "$ROOTFS_SRC" >/tmp/mixtar-0069-unsquashfs.log 2>&1 || {
	cat /tmp/mixtar-0069-unsquashfs.log
	exit 1
}

sudo -n mkdir -p "$ROOT_WORK/root/dev" "$ROOT_WORK/root/proc" "$ROOT_WORK/root/sys" "$ROOT_WORK/root/run"
...
LOG=$BASE_LOG_DIR/0069-after-switch-root.log
...
write_snapshot() {
	{
		echo "stage=0069-after-switch-root"
		echo "pid=$$"
...
mkdir -p "$BASE_LOG_DIR" 2>/dev/null || true
append_log "0069 diagnostic pid1 started"
...
write_snapshot
append_log "0069 diagnostic pid1 syncing and rebooting to firmware default"
sync
sleep 8
...
rootfs_mode=image-readonly-pid1-log-autoreboot-rw
...
diagnostic_mountpoints_present=true
EOS

sudo -n mksquashfs "$ROOT_WORK/root" "$ROOTFS_OUT" -noappend -no-progress >/tmp/mixtar-0069-mksquashfs.log 2>&1 || {
	cat /tmp/mixtar-0069-mksquashfs.log
	exit 1
}
...
verify_rootfs_init=$(sudo -n grep -n -E '0069-after-switch-root|sysrq-trigger|reboot -f|tty0|diagnostic pid1' "$VERIFY/rootfs/sbin/init" 2>/dev/null || true)
verify_mountpoints=$(sudo -n ls -ld "$VERIFY/rootfs/dev" "$VERIFY/rootfs/proc" "$VERIFY/rootfs/sys" "$VERIFY/rootfs/run" 2>/dev/null || true)
sudo -n umount "$VERIFY/rootfs"

printf '%s\n' "$verify_rootfs_marker" | grep -q 'diagnostic_pid1_autoreboot=true' || {
	echo "rootfs 0022 autoreboot marker missing" >&2
	exit 1
}
printf '%s\n' "$verify_rootfs_marker" | grep -q 'diagnostic_mountpoints_present=true' || {
	echo "rootfs 0022 mountpoint marker missing" >&2
	exit 1
}
printf '%s\n' "$verify_mountpoints" | grep -q '/dev' || {
	echo "rootfs 0022 mountpoint verification failed" >&2
	exit 1
}

sudo -n sh -c "cd '$INIT_WORK' && gzip -dc '$INITRD_SRC' | cpio -idmu" >/tmp/mixtar-0069-cpio-in.log 2>&1 || {
	cat /tmp/mixtar-0069-cpio-in.log
	exit 1
}

HANDOFF=$INIT_WORK/usr/bin/mixtar-initramfs-handoff
need_file "$HANDOFF"
sudo -n cp "$HANDOFF" "$HANDOFF.before-0069"
...
sudo -n awk '
	...
	in_mount_base == 1 && /^\tfi$/ {
		...
		print "\t{"
		print "\t\techo \"stage=0069-after-base-root-mount\""
		print "\t\techo \"pid=$$\""
		...
		print "\t\tprobe=\"$base_diag/0069-rw-probe.$$\""
		...
		print "\t} >> \"$base_diag/0069-initramfs-handoff.log\" 2>&1 || true"
		print "\tsync"
		in_mount_base = 0
		next
	}
...
verify_patch=$(sudo -n grep -n -E '0069-after-base-root-mount|0069-initramfs-handoff|PRIMARY_INIT=/sbin/init|0067-before-switch-root|switch_root' "$HANDOFF" || true)
printf '%s\n' "$verify_patch" | grep -q '0069-initramfs-handoff' || {
	echo "0069 handoff rw logging patch verification failed" >&2
	exit 1
}
printf '%s\n' "$verify_patch" | grep -q 'PRIMARY_INIT=/sbin/init' || {
	echo "0069 primary init verification failed" >&2
	exit 1
}

sudo -n sh -c "cd '$INIT_WORK' && find . | cpio -o -H newc | gzip -9 > '$INITRD_OUT'" >/tmp/mixtar-0069-cpio-out.log 2>&1 || {
	cat /tmp/mixtar-0069-cpio-out.log
	exit 1
}
...
sudo -n sh -c "cd '$VERIFY/initrd' && gzip -dc '$INITRD_OUT' | cpio -idmu" >/tmp/mixtar-0069-verify-initrd.log 2>&1 || {
	cat /tmp/mixtar-0069-verify-initrd.log
	exit 1
}

verify_handoff=$(sudo -n grep -n -E '0069-after-base-root-mount|0069-initramfs-handoff|PRIMARY_INIT=/sbin/init|0067-before-switch-root|busybox switch_root' "$VERIFY/initrd/usr/bin/mixtar-initramfs-handoff" || true)
printf '%s\n' "$verify_handoff" | grep -q '0069-initramfs-handoff' || {
	echo "verified initrd 0069 rw logging patch missing" >&2
	exit 1
}
...
sudo -n efibootmgr -c -d /dev/nvme0n1 -p 1 -L "$LABEL" -l "$LOADER" -u "$cmdline" >/tmp/mixtar-0069-efibootmgr-create.log 2>&1 || {
	cat /tmp/mixtar-0069-efibootmgr-create.log
	exit 1
}
...
sudo -n efibootmgr -o "$before_order" >/tmp/mixtar-0069-efibootmgr-order.log 2>&1 || {
	cat /tmp/mixtar-0069-efibootmgr-order.log
	exit 1
}
...
{
	echo "stage=0069-pid1-log-autoreboot-rw"
	...
	echo "next_required_stage=0070-set-bootnext-one-shot-pid1-log-autoreboot-rw-test"
