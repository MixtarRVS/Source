...
ROOTFS_SRC=/System/Generations/0023-rootfs-image-openrc-watchdog/rootfs.squashfs
ROOTFS_GEN=/System/Generations/0024-rootfs-image-openrc-direct-supervisor
ROOTFS_OUT=$ROOTFS_GEN/rootfs.squashfs

REPORT=/System/Base/Closure/0071-openrc-direct-supervisor-watchdog.report
ROOT_WORK=/tmp/mixtar-rootfs-0024.$$
VERIFY=/tmp/mixtar-0071-verify.$$

ESP_DEV=/dev/nvme0n1p1
ESP_MOUNT=/tmp/mixtar-esp-0071

LABEL="MixtarRVS RT Candidate 0071 openrc direct supervisor"
LOADER="\\EFI\\mixtarrvs-rt\\vmlinuz.efi"
INITRD="\\EFI\\mixtarrvs-rt\\initrd-mixtar-candidate-0069.img"
ROOTFS_ARG=/System/Generations/0024-rootfs-image-openrc-direct-supervisor/rootfs.squashfs
...
sudo -n unsquashfs -d "$ROOT_WORK/root" "$ROOTFS_SRC" >/tmp/mixtar-0071-unsquashfs.log 2>&1 || {
	cat /tmp/mixtar-0071-unsquashfs.log
	exit 1
}
...
LOG=$BASE_LOG_DIR/0071-after-switch-root.log
OPENRC_LOG=$BASE_LOG_DIR/0071-openrc.log
...
write_snapshot() {
	{
		echo "stage=0071-after-switch-root"
		echo "pid=$$"
		...
		echo "tools_end"
		echo "services_begin"
		ls -l /etc/runlevels/default 2>&1 || true
		echo "services_end"
...
start_watchdog() {
	(
		...
		mkdir -p "$BASE_LOG_DIR" 2>/dev/null || true
		{
			echo "stage=0071-watchdog-reboot"
			echo "reason=timeout"
			echo "cmdline=$(cat /proc/cmdline 2>/dev/null || true)"
			...
			cat /proc/mounts 2>/dev/null || true
			echo "mounts_end"
		} >>"$BASE_LOG_DIR/0071-watchdog-reboot.log" 2>&1 || true
		sync
...
mkdir -p "$BASE_LOG_DIR" 2>/dev/null || true
append_log "0071 direct supervisor pid1 started"
...
if [ -x /sbin/openrc ]; then
	append_log "0071 running OpenRC directly"
	run_openrc_stage sysinit
	run_openrc_stage boot
	run_openrc_stage default
else
	append_log "0071 /sbin/openrc missing"
fi

{
	echo "stage=0071-after-openrc"
	echo "rc_status_begin"
	rc-status 2>/dev/null || true
	echo "rc_status_end"
	echo "ip_addr_begin"
	...
	ps 2>/dev/null || true
	echo "ps_end"
} >>"$BASE_LOG_DIR/0071-after-openrc.log" 2>&1 || true
sync

append_log "0071 supervisor entering idle loop"
while :; do
	sleep 60
done
EOS
...
rootfs_mode=image-readonly-openrc-direct-supervisor
source_generation=0023-rootfs-image-openrc-watchdog
base_preserve_mountpoint=/System/Runtime/initramfs/base
init_wrapper=/sbin/init
init_behavior=direct-openrc-supervisor
watchdog_reboot_seconds=240
runtime_mounts=devtmpfs,proc,sysfs,devpts,run-tmpfs,tmp-tmpfs
EOS

sudo -n mksquashfs "$ROOT_WORK/root" "$ROOTFS_OUT" -noappend -no-progress >/tmp/mixtar-0071-mksquashfs.log 2>&1 || {
	cat /tmp/mixtar-0071-mksquashfs.log
	exit 1
}
...
verify_rootfs_init=$(sudo -n grep -n -E '0071-after-switch-root|direct supervisor|openrc|watchdog|tty0' "$VERIFY/rootfs/sbin/init" 2>/dev/null || true)
...
printf '%s\n' "$verify_rootfs_marker" | grep -q 'rootfs_mode=image-readonly-openrc-direct-supervisor' || {
	echo "rootfs 0024 direct supervisor marker missing" >&2
	exit 1
}
printf '%s\n' "$verify_rootfs_init" | grep -q 'direct supervisor' || {
	echo "rootfs 0024 direct supervisor init verification failed" >&2
	exit 1
}
...
sudo -n efibootmgr -c -d /dev/nvme0n1 -p 1 -L "$LABEL" -l "$LOADER" -u "$cmdline" >/tmp/mixtar-0071-efibootmgr-create.log 2>&1 || {
	cat /tmp/mixtar-0071-efibootmgr-create.log
	exit 1
}
...
sudo -n efibootmgr -o "$before_order" >/tmp/mixtar-0071-efibootmgr-order.log 2>&1 || {
	cat /tmp/mixtar-0071-efibootmgr-order.log
	exit 1
}
...
{
	echo "stage=0071-openrc-direct-supervisor-watchdog"
	...
	echo "init_behavior=direct-openrc-supervisor"
	echo "watchdog_reboot_seconds=240"
	echo "label=$LABEL"
	...
	echo "next_required_stage=0072-set-bootnext-one-shot-openrc-direct-supervisor-test"
