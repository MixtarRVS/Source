...
ROOTFS_SRC=/System/Generations/0024-rootfs-image-openrc-direct-supervisor/rootfs.squashfs
ROOTFS_GEN=/System/Generations/0025-rootfs-image-network-persistent-supervisor
ROOTFS_OUT=$ROOTFS_GEN/rootfs.squashfs

REPORT=/System/Base/Closure/0072-network-persistent-supervisor.report
ROOT_WORK=/tmp/mixtar-rootfs-0025.$$
VERIFY=/tmp/mixtar-0072-verify.$$

ESP_DEV=/dev/nvme0n1p1
ESP_MOUNT=/tmp/mixtar-esp-0072

LABEL="MixtarRVS RT Candidate 0072 network persistent supervisor"
LOADER="\\EFI\\mixtarrvs-rt\\vmlinuz.efi"
INITRD="\\EFI\\mixtarrvs-rt\\initrd-mixtar-candidate-0069.img"
ROOTFS_ARG=/System/Generations/0025-rootfs-image-network-persistent-supervisor/rootfs.squashfs
...
sudo -n unsquashfs -d "$ROOT_WORK/root" "$ROOTFS_SRC" >/tmp/mixtar-0072-unsquashfs.log 2>&1 || {
	cat /tmp/mixtar-0072-unsquashfs.log
	exit 1
}
...
LOG=$BASE_LOG_DIR/0072-after-switch-root.log
OPENRC_LOG=$BASE_LOG_DIR/0072-openrc.log
...
write_snapshot() {
	{
		echo "stage=0072-after-switch-root"
		echo "pid=$$"
...
network_ready() {
	ip addr show wlan0 2>/dev/null | grep -q 'inet 192\\.168\\.99\\.' || return 1
...
start_watchdog() {
	(
		...
		while [ "$count" -lt 60 ]; do
			if [ -f /run/mixtar-network-ready ]; then
				exit 0
			fi
			sleep 5
			count=$((count + 1))
		...
		mkdir -p "$BASE_LOG_DIR" 2>/dev/null || true
		{
			echo "stage=0072-watchdog-reboot"
			...
			ps 2>/dev/null || true
			echo "ps_end"
		} >>"$BASE_LOG_DIR/0072-watchdog-reboot.log" 2>&1 || true
		sync
...
mkdir -p "$BASE_LOG_DIR" 2>/dev/null || true
append_log "0072 persistent network supervisor pid1 started"
...
if [ -x /sbin/openrc ]; then
	append_log "0072 running OpenRC directly"
	run_openrc_stage sysinit
	run_openrc_stage boot
	run_openrc_stage default
else
	append_log "0072 /sbin/openrc missing"
fi
...
{
	echo "stage=0072-after-openrc-network"
	...
	ps 2>/dev/null || true
	echo "ps_end"
} >>"$BASE_LOG_DIR/0072-after-openrc-network.log" 2>&1 || true
sync

append_log "0072 supervisor entering idle loop"
while :; do
	sleep 60
done
EOS
...
rootfs_mode=image-readonly-network-persistent-supervisor
source_generation=0024-rootfs-image-openrc-direct-supervisor
base_preserve_mountpoint=/System/Runtime/initramfs/base
init_wrapper=/sbin/init
init_behavior=direct-openrc-supervisor-network-persistent
...
sudo -n mksquashfs "$ROOT_WORK/root" "$ROOTFS_OUT" -noappend -no-progress >/tmp/mixtar-0072-mksquashfs.log 2>&1 || {
	cat /tmp/mixtar-0072-mksquashfs.log
	exit 1
}
...
verify_rootfs_init=$(sudo -n grep -n -E '0072-after-switch-root|persistent network|bind_dir|network_ready|mixtar-network-ready' "$VERIFY/rootfs/sbin/init" 2>/dev/null || true)
...
printf '%s\n' "$verify_rootfs_marker" | grep -q 'rootfs_mode=image-readonly-network-persistent-supervisor' || {
	echo "rootfs 0025 network persistent marker missing" >&2
	exit 1
}
...
sudo -n efibootmgr -c -d /dev/nvme0n1 -p 1 -L "$LABEL" -l "$LOADER" -u "$cmdline" >/tmp/mixtar-0072-efibootmgr-create.log 2>&1 || {
	cat /tmp/mixtar-0072-efibootmgr-create.log
	exit 1
}
...
sudo -n efibootmgr -o "$before_order" >/tmp/mixtar-0072-efibootmgr-order.log 2>&1 || {
	cat /tmp/mixtar-0072-efibootmgr-order.log
	exit 1
}
...
{
	echo "stage=0072-network-persistent-supervisor"
	...
	echo "init_behavior=direct-openrc-supervisor-network-persistent"
	...
	echo "next_required_stage=0073-set-bootnext-one-shot-network-persistent-supervisor-test"
