...
ROOTFS_SRC=/System/Generations/0026-rootfs-image-network-ready-supervisor/rootfs.squashfs
ROOTFS_GEN=/System/Generations/0027-rootfs-image-users-users-network-ready-supervisor
ROOTFS_OUT=$ROOTFS_GEN/rootfs.squashfs

REPORT=/System/Base/Closure/0074-users-network-ready-supervisor.report
ROOT_WORK=/tmp/mixtar-rootfs-0027.$$
VERIFY=/tmp/mixtar-0074-verify.$$

ESP_DEV=/dev/nvme0n1p1
ESP_MOUNT=/tmp/mixtar-esp-0074

LABEL="MixtarRVS RT Candidate 0074 users network ready supervisor"
LOADER="\\EFI\\mixtarrvs-rt\\vmlinuz.efi"
INITRD="\\EFI\\mixtarrvs-rt\\initrd-mixtar-candidate-0069.img"
ROOTFS_ARG=/System/Generations/0027-rootfs-image-users-users-network-ready-supervisor/rootfs.squashfs
...
sudo -n unsquashfs -d "$ROOT_WORK/root" "$ROOTFS_SRC" >/tmp/mixtar-0074-unsquashfs.log 2>&1 || {
	cat /tmp/mixtar-0074-unsquashfs.log
	exit 1
}

sudo -n mkdir -p "$ROOT_WORK/root/dev" "$ROOT_WORK/root/proc" "$ROOT_WORK/root/sys" "$ROOT_WORK/root/run" "$ROOT_WORK/root/tmp"
sudo -n mkdir -p "$ROOT_WORK/root/var/lib/iwd" "$ROOT_WORK/root/var/lib/dhcpcd" "$ROOT_WORK/root/var/log" "$ROOT_WORK/root/Users"
...
LOG=$BASE_LOG_DIR/0074-after-switch-root.log
OPENRC_LOG=$BASE_LOG_DIR/0074-openrc.log
...
write_snapshot() {
	{
		echo "stage=0074-after-switch-root"
		echo "pid=$$"
...
start_watchdog() {
	(
		...
		while [ "$count" -lt 60 ]; do
			if [ -f /run/mixtar-ssh-confirmed ]; then
				exit 0
			fi
		...
		mkdir -p "$BASE_LOG_DIR" 2>/dev/null || true
		{
			echo "stage=0074-watchdog-reboot"
			echo "reason=ssh-confirmation-timeout"
			echo "cmdline=$(cat /proc/cmdline 2>/dev/null || true)"
			...
			ps 2>/dev/null || true
			echo "ps_end"
		} >>"$BASE_LOG_DIR/0074-watchdog-reboot.log" 2>&1 || true
		sync
...
mkdir -p "$BASE_LOG_DIR" 2>/dev/null || true
append_log "0074 users users network ready supervisor pid1 started"
...
if [ -x /sbin/openrc ]; then
	append_log "0074 running OpenRC directly"
	run_openrc_stage sysinit
	run_openrc_stage boot
	run_openrc_stage default
else
	append_log "0074 /sbin/openrc missing"
fi
...
{
	echo "stage=0074-after-openrc-network"
	...
	ps 2>/dev/null || true
	echo "ps_end"
} >>"$BASE_LOG_DIR/0074-after-openrc-network.log" 2>&1 || true
sync

append_log "0074 supervisor entering idle loop"
while :; do
	sleep 60
done
EOS
...
rootfs_mode=image-readonly-users-network-ready-supervisor
source_generation=0026-rootfs-image-network-ready-supervisor
base_preserve_mountpoint=/System/Runtime/initramfs/base
init_wrapper=/sbin/init
init_behavior=direct-openrc-supervisor-users-network-ready
watchdog_reboot_seconds=300
network_state_binds=/var/lib/iwd,/var/lib/dhcpcd,/System/Logs,/var/log,/Users
runtime_mounts=devtmpfs,proc,sysfs,devpts,run-tmpfs,tmp-tmpfs
EOS

sudo -n mksquashfs "$ROOT_WORK/root" "$ROOTFS_OUT" -noappend -no-progress >/tmp/mixtar-0074-mksquashfs.log 2>&1 || {
	cat /tmp/mixtar-0074-mksquashfs.log
	exit 1
}
...
verify_rootfs_init=$(sudo -n grep -n -E '0074-after-switch-root|persistent network|bind_dir|network_ready|mixtar-network-ready' "$VERIFY/rootfs/sbin/init" 2>/dev/null || true)
verify_mountpoints=$(sudo -n ls -ld "$VERIFY/rootfs/var/lib/iwd" "$VERIFY/rootfs/var/lib/dhcpcd" "$VERIFY/rootfs/System/Logs" "$VERIFY/rootfs/var/log" "$VERIFY/rootfs/Users" 2>/dev/null || true)
sudo -n umount "$VERIFY/rootfs"

printf '%s\n' "$verify_rootfs_marker" | grep -q 'rootfs_mode=image-readonly-users-network-ready-supervisor' || {
	echo "rootfs 0025 users network ready marker missing" >&2
	exit 1
}
...
sudo -n efibootmgr -c -d /dev/nvme0n1 -p 1 -L "$LABEL" -l "$LOADER" -u "$cmdline" >/tmp/mixtar-0074-efibootmgr-create.log 2>&1 || {
	cat /tmp/mixtar-0074-efibootmgr-create.log
	exit 1
}
...
sudo -n efibootmgr -o "$before_order" >/tmp/mixtar-0074-efibootmgr-order.log 2>&1 || {
	cat /tmp/mixtar-0074-efibootmgr-order.log
	exit 1
}
...
{
	echo "stage=0074-users-network-ready-supervisor"
	...
	echo "init_behavior=direct-openrc-supervisor-users-network-ready"
	...
	echo "next_required_stage=0074-set-bootnext-one-shot-users-network-ready-supervisor-test"
