...
STAGE="0077"
LABEL="MixtarRVS RT Candidate 0077 runtime closure confirmed supervisor"
...
SRC="$SYSTEM_ROOT/Generations/0029-rootfs-image-runtime-closure-supervisor/rootfs.squashfs"
if [ ! -f "$SRC" ]; then
  SRC="$SYSTEM_ROOT/Generations/0028-rootfs-image-ssh-auth-diagnostic-supervisor/rootfs.squashfs"
fi

TARGET_DIR="$SYSTEM_ROOT/Generations/0030-rootfs-image-runtime-closure-confirmed-supervisor"
TARGET="$TARGET_DIR/rootfs.squashfs"
WORK="/tmp/mixtar-stage-0077"
ESP="/tmp/mixtar-esp-0077"
...
safe_rm_work() {
  case "$1" in
    /tmp/mixtar-stage-0077|/tmp/mixtar-stage-0077/*) rm -rf "$1" ;;
...
write_stage_init() {
  ...
  cat > "$root/System/Base/Closure/0077-runtime-closure-confirmed.manifest" <<'MANIFEST'
stage=0077
identity=MixtarRVS
...
LOG="$LOG_DIR/0077-runtime-closure-confirmed.log"
...
: > "$LOG" 2>/dev/null || true
write_log "mixtar $STAGE runtime closure confirmed supervisor started"
...
echo "TARGET=$TARGET"
echo "LOG_AFTER_BOOT=/System/Runtime/initramfs/base/System/Base/Closure/0077-runtime-closure-confirmed.log"
