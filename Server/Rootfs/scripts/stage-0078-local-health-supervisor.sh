...
STAGE="0078"
LABEL="MixtarRVS RT Candidate 0078 local health supervisor"
...
TARGET_DIR="$SYSTEM_ROOT/Generations/0031-rootfs-image-local-health-supervisor"
TARGET="$TARGET_DIR/rootfs.squashfs"
WORK="/tmp/mixtar-stage-0078"
ESP="/tmp/mixtar-esp-0078"
...
safe_rm_work() {
  case "$1" in
    /tmp/mixtar-stage-0078|/tmp/mixtar-stage-0078/*) rm -rf "$1" ;;
...
write_stage_init() {
  ...
  cat > "$root/System/Base/Closure/0078-local-health.manifest" <<'MANIFEST'
stage=0078
identity=MixtarRVS
...
LOG="$LOG_DIR/0078-local-health.log"
...
health_gate() {
  ...
  status="$(rc-status 2>/dev/null || true)"
  for svc in iwd dhcpcd sshd; do
    if ! printf '%s\n' "$status" | grep -q "$svc.*started"; then
      failures="$failures service-$svc"
    fi
  done
  if [ -z "$failures" ]; then
...
echo "TARGET=$TARGET"
echo "LOG_AFTER_BOOT=/System/Runtime/initramfs/base/System/Base/Closure/0078-local-health.log"
