...
STAGE="0081"
LABEL="MixtarRVS RT Candidate 0081 hard trace pid1 supervisor"
...
TARGET_DIR="$SYSTEM_ROOT/Generations/0033-rootfs-image-hard-trace-pid1-supervisor"
TARGET="$TARGET_DIR/rootfs.squashfs"
WORK="/tmp/mixtar-stage-0081"
ESP="/tmp/mixtar-esp-0081"
...
safe_rm_work() {
  case "$1" in
    /tmp/mixtar-stage-0081|/tmp/mixtar-stage-0081/*) rm -rf "$1" ;;
...
write_stage_init() {
  ...
  cat > "$root/System/Base/Closure/0081-hard-trace-pid1.manifest" <<'MANIFEST'
stage=0081
identity=MixtarRVS
...
mount_basic() {
  ...
  mkdir -p /run/openrc /run/lock /var/run 2>/dev/null
...
snapshot() {
  {
    ...
    echo "dev="
    ls -l /dev/console /dev/null /dev/tty /dev/tty0 /dev/tty1 2>/dev/null
    echo "user="
    getent passwd vxz 2>/dev/null || grep '^vxz:' /etc/passwd 2>/dev/null
    echo "network="
...
start_watchdog() {
  ...
  (
    ...
    printf '0081 watchdog timeout; rebooting fallback\n' >> "$LOG" 2>/dev/null
    sync
    reboot -f
  ) &
}
...
health_gate() {
  ...
  listen="$(ss -ltn 2>/dev/null || netstat -ltn 2>/dev/null)"
...
log "phase pid1_before_mount_basic"
mount_basic
start_watchdog
bind_state
snapshot
run_phase /sbin/openrc sysinit
run_phase /sbin/openrc boot
run_phase /sbin/openrc default
...
echo "TARGET=$TARGET"
echo "LOG_AFTER_BOOT=/System/Runtime/initramfs/base/System/Base/Closure/0081-hard-trace-pid1.log"
