...
STAGE=0039-copy-pathlog-candidate-to-esp-no-active-switch
BASE=/System/Base/Closure/$STAGE
TOOL_SRC=${1:-/tmp/mixtar-pathlog-candidate-esp-efi.sh}
MANIFEST_SRC=${2:-/tmp/base-closure-0039-copy-pathlog-candidate-to-esp-no-active-switch.json}
TOOL_TARGET=/System/Initramfs/Prototypes/mixtar-pathlog-candidate-esp-efi.sh
...
[ -n "$bootnext" ] && bootnext_present=true || bootnext_present=false
[ "$status" = verified ] && stage_status=verified || stage_status=needs_attention

cat > "$BASE/pathlog-candidate-esp-efi-summary.txt" <<EOF
stage=$STAGE
status=$stage_status
...
cat "$BASE/pathlog-candidate-esp-efi-summary.txt" > "$BASE/report.txt"
