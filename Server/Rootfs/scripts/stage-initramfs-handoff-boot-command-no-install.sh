...
STAGE=0029-initramfs-handoff-boot-command-no-install
BASE=/System/Base/Closure/$STAGE
BUILDER_NAME=mixtar-initramfs-handoff-boot-builder.sh
HANDOFF_NAME=mixtar-initramfs-handoff-prototype.sh
BUILDER_TARGET=/System/Initramfs/Prototypes/mixtar-initramfs-handoff-boot-builder.sh
HANDOFF_TARGET=/System/Initramfs/Prototypes/mixtar-initramfs-handoff.sh
...
write_contract() {
	cat <<EOF
MixtarRVS initramfs handoff boot command, stage 0029
...
This stage is non-activating:
  handoff boot command is packaged but not executed
...
write_summary() {
	cat > "$BASE/initramfs-handoff-boot-command-summary.txt" <<EOF
initramfs_handoff_boot_command_status=generated
builder_target=$BUILDER_TARGET
handoff_target=$HANDOFF_TARGET
target_image=$(value_from_file "$BASE/build-candidate.txt" target_image)
target_size_bytes=$(value_from_file "$BASE/build-candidate.txt" target_size_bytes)
target_sha256=$(value_from_file "$BASE/build-candidate.txt" target_sha256)
source_candidate_sha256=$(value_from_file "$BASE/build-candidate.txt" source_candidate_sha256)
handoff_source_sha256=$(value_from_file "$BASE/build-candidate.txt" handoff_source_sha256)
...
handoff_hash_match=$(value_from_file "$BASE/verify.txt" handoff_hash_match)
boot_command_available=$(value_from_file "$BASE/verify.txt" boot_command_available)
boot_executes_switch_root_when_run=$(value_from_file "$BASE/verify.txt" boot_executes_switch_root_when_run)
handoff_boot_candidate_ready=$(value_from_file "$BASE/report.txt" handoff_boot_candidate_ready)
boot_entry_ready=$(value_from_file "$BASE/report.txt" boot_entry_ready)
...
verify_stage() {
	rc=0
	for file in contract.txt plan.txt build-candidate.txt inspect.txt verify.txt report.txt initramfs-handoff-boot-command-summary.txt; do
		if [ ! -s "$BASE/$file" ]; then
	...
	for token in \
		...
		'target_contains=etc/mixtar-initramfs-handoff-boot-candidate:true'
	do
	...
	for token in \
		'target_handoff_contract_runs=true' \
...
stage() {
	...
	handoff=$(source_file "$HANDOFF_NAME") || return 1
	...
	if is_mounted; then
		printf 'stage: mountpoint already mounted before handoff boot candidate build: %s\n' "$MOUNT_POINT" >&2
		return 1
	fi
	if [ "$builder" != "$BASE/$BUILDER_NAME" ]; then
		install -m 0755 "$builder" "$BASE/$BUILDER_NAME"
	fi
	if [ "$handoff" != "$BASE/$HANDOFF_NAME" ]; then
		install -m 0755 "$handoff" "$BASE/$HANDOFF_NAME"
	fi
	install -m 0755 "$builder" "$BUILDER_TARGET"
	install -m 0755 "$handoff" "$HANDOFF_TARGET"
	...
	if verify_stage; then
		printf 'verified\n' > "$BASE/initramfs-handoff-boot-command-no-install-status.txt"
	else
		printf 'incomplete\n' > "$BASE/initramfs-handoff-boot-command-no-install-status.txt"
		return 1
	fi
