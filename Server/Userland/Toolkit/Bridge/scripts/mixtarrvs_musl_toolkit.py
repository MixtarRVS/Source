...
def musl_command(tool: str) -> str:
    cmd = plan_for(tool)
    cmd = cmd.replace("cp Server/Userland/Toolkit/Bridge/scripts/", "/bin/cp Server/Userland/Toolkit/Bridge/scripts/")
    cmd = cmd.replace(" && chmod +x ", " && /bin/chmod 0755 ")
    cmd = cmd.replace(
        "Server/Userland/Generated/targets/linux-x64/bin",
        "out/mixtarrvs-musl-target/bin",
    )
    cmd = cmd.replace(
        "Server/Userland/Generated/targets/linux-x64/libexec",
