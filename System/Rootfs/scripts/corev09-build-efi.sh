...
stage_root_script="$repo_root/System/Rootfs/scripts/corev09-stage-root.sh"
...
boot_dir="$repo_root/System/Kernel/Generated/boot"
fragment="$repo_root/System/Kernel/configs/mixtar-corev09-rt.fragment"
firmware_dir="$repo_root/System/Kernel/Generated/firmware"
...
stage_root="$repo_root/System/Rootfs/Generated/corev09-root"
work_dir="$repo_root/System/Rootfs/Generated/corev09-efi-build"
...
repo_snapshot_archive="$repo_root/System/Kernel/Generated/sources/linux-$KERNEL_VERSION.snapshot.tar.gz"
initramfs_cpio="$work_dir/corev09-initramfs.cpio"
efi_artifact="$work_dir/MixtarRVS-$VERSION.efi"
provenance_file="$efi_artifact.provenance"
cmdline_file="$work_dir/corev09.cmdline"
...
cmdline_extra="${COREV09_CMDLINE_EXTRA:-}"
if [[ "${COREV09_AUTORETURN:-0}" == "1" ]]; then
...
usage() {
  cat <<EOF
usage: corev09-build-efi.sh [plan|fetch|prepare|build|stage|verify|import PATH|all]

Builds the CoreV09 single EFI artifact locally:
  /System/EFI/MixtarRVS/$VERSION.efi

It writes only under:
  case-sensitive kernel workspace
  System/Rootfs/Generated
  System/Kernel/Generated/boot
...
fail() {
  echo "corev09-build-efi: error: $*" >&2
  exit 1
}

note() {
  echo "corev09-build-efi: $*"
}
...
ensure_case_sensitive_kernel_tree() {
  ...
  [[ "$upper_inode" != "$lower_inode" ]] || fail "kernel source is on a case-insensitive filesystem; set MIXTARRVS_COREV09_KERNEL_WORKSPACE to a WSL/ext4 path"
}
...
stage_root_if_needed() {
  note "staging CoreV09 root before initramfs"
  MIXTARRVS_COREV09_KERNEL_WORKSPACE="$kernel_workspace" \
